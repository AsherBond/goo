...
-   [Twitter](https://tropicaldevel.wordpress.com/2013/01/16/openstack-glance-and-nova-backed-by-red-hat-storage/?share=twitter "Click to share on Twitter")
-   [Facebook](https://tropicaldevel.wordpress.com/2013/01/16/openstack-glance-and-nova-backed-by-red-hat-storage/?share=facebook "Share on Facebook")
-   
...
Posted in
[OpenStack](https://tropicaldevel.wordpress.com/category/openstack/ "View all posts in OpenStack")

« [An Image Transfers Service
For OpenStack](https://tropicaldevel.wordpress.com/2013/01/11/an-image-transfers-service-for-openstack/)

[Debugging OpenStack with pycharm
and pydevd](https://tropicaldevel.wordpress.com/2013/01/24/debugging-openstack-with-pycharm-and-pydevd/)
»
...
1.  ![](https://i0.wp.com/graph.facebook.com/1030394194/picture?q=type%3Dlarge%26_md5%3Dc12bb5c6654e0723cebdc1c8387c9d1c&resize=48%2C48)

    What were the specific issues that required SELinux to be disabled?

    For mounting the gluster volume, perhaps show /etc/fstab (since
    mount -t won’t be persistent)

    And for using glusterfs to back Nova instances, does this mean that
    a single base image in /var/lib/nova/instances/\_base would be
    shared among multiple compute nodes? (Side benefit of this is that
    only a single retrieval from glance is needed even if the same base
    image is launched on multiple Compute Nodes, right?)

    Finally… Does having a gluster backed Nova instance store allow you
    to do live migration of VMs or is that restricted to only when
    Nova/Cinder volumes are used for guest boot disks?

    -   

    By: **[Perry Myers](http://www.facebook.com/pmyers)** on January 16,
    2013 \
     at 8:47 pm

    [Reply](/2013/01/16/openstack-glance-and-nova-backed-by-red-hat-storage/?replytocom=16#respond)

    -   ![](https://2.gravatar.com/avatar/2de3b88e529ad34e021a0441d3cf63fb?s=48&d=identicon&r=G)

        In order to run with selinux in enforcing mode (which I agree is
        the right thing to do) see this bug (in particular comment 6 and
        7)
        [https://bugzilla.redhat.com/show\_bug.cgi?id=876452](https://bugzilla.redhat.com/show_bug.cgi?id=876452)
        ...
        [Reply](/2013/01/16/openstack-glance-and-nova-backed-by-red-hat-storage/?replytocom=18#respond)

2.  ![](https://2.gravatar.com/avatar/2de3b88e529ad34e021a0441d3cf63fb?s=48&d=identicon&r=G)
    ...
    There is a solution for Red Hat Storage build clients, but not for
    RHEL 6.4. Obviously this is something a real system would need
    because the file system needs to come up before nova-compute does or
    there will be errors. As a temporary work around you can add this
    line to /etc/init.d/openstack-keystone at line 95 (i will paste the
    line before it and after it for context):

    echo -n \$”Starting \$prog: ”\
     mount -t glusterfs 192.168.50.223:/testvol /mnt/gluster/ &\>
    /tmp/error\_keystone\_gluster.log\
     daemon –user keystone –pidfile \$pidfile “\$exec –config-file
    \$config &\>/dev/null & echo \\\$! \> \$pidfile”
    ...
    [Reply](/2013/01/16/openstack-glance-and-nova-backed-by-red-hat-storage/?replytocom=17#respond)

3.  ![](https://2.gravatar.com/avatar/2de3b88e529ad34e021a0441d3cf63fb?s=48&d=identicon&r=G)
    ...
    gluster client on Fedora would have to manually be upgraded, or the
    NFS client could be used.

    -   

    By: **[John Bresnahan](http://gravatar.com/buzztroll)** on January
    18, 2013 \
     at 11:49 pm

    [Reply](/2013/01/16/openstack-glance-and-nova-backed-by-red-hat-storage/?replytocom=24#respond)

4.  ![](https://1.gravatar.com/avatar/d4589085fd79c9d4b3ebdebd1e877a8e?s=48&d=identicon&r=G)
    ...
    \# gluster volume create testvol myhost:/exp1\
     Creation of volume testvol has been successful. Please start the
    ...
    [Reply](/2013/01/16/openstack-glance-and-nova-backed-by-red-hat-storage/?replytocom=105#respond)

\

### Leave a Reply [Cancel reply](/2013/01/16/openstack-glance-and-nova-backed-by-red-hat-storage/#respond) {#reply-title}
...
( [Log Out](javascript:HighlanderComments.doExternalLogout(%20'wordpress'%20);) / [Change](#) )

![Twitter
picture](https://1.gravatar.com/avatar/ad516503a11cd5ca435acc9bb6523536?s=25&d=identicon&forcedefault=y&r=G)

**** You are commenting using your Twitter account.
( [Log Out](javascript:HighlanderComments.doExternalLogout(%20'twitter'%20);) / [Change](#) )

![Facebook
photo](https://1.gravatar.com/avatar/ad516503a11cd5ca435acc9bb6523536?s=25&d=identicon&forcedefault=y&r=G)

**** You are commenting using your Facebook account.
( [Log Out](javascript:HighlanderComments.doExternalLogout(%20'facebook'%20);) / [Change](#) )
...
-   [OpenStack](https://tropicaldevel.wordpress.com/category/openstack/ "Posts about OpenStack software development.")
-   [Uncategorized](https://tropicaldevel.wordpress.com/category/uncategorized/ "View all posts filed under Uncategorized")

Recent Posts
------------

-   [Free Cache](https://tropicaldevel.wordpress.com/2013/04/09/free-cache/ "Free Cache")
-   [The Cost Of Client Side Image Downloads On
    the CPU](https://tropicaldevel.wordpress.com/2013/03/26/the-cost-of/ "The Cost Of Client Side Image Downloads On the CPU")
-   [A Look At Performance When Glance Is Backed
    By Gluster](https://tropicaldevel.wordpress.com/2013/03/06/a-look-at-performance-when-glance-is-backed-by-gluster/ "A Look At Performance When Glance Is Backed By Gluster")
-   [Debugging OpenStack with pycharm
    and pydevd](https://tropicaldevel.wordpress.com/2013/01/24/debugging-openstack-with-pycharm-and-pydevd/ "Debugging OpenStack with pycharm and pydevd")
-   [OpenStack Glance And Nova Backed By Red
    Hat Storage](https://tropicaldevel.wordpress.com/2013/01/16/openstack-glance-and-nova-backed-by-red-hat-storage/ "OpenStack Glance And Nova Backed By Red Hat Storage")

Archives
--------

-   [April
    2013](https://tropicaldevel.wordpress.com/2013/04/ "April 2013")
-   [March
    2013](https://tropicaldevel.wordpress.com/2013/03/ "March 2013")
-   [January
    2013](https://tropicaldevel.wordpress.com/2013/01/ "January 2013")
-   [December
    2012](https://tropicaldevel.wordpress.com/2012/12/ "December 2012")
-   [September
    2012](https://tropicaldevel.wordpress.com/2012/09/ "September 2012")
...
-   [Register](https://tropicaldevel.wordpress.com/wp-login.php?action=register)
-   [Log in](https://tropicaldevel.wordpress.com/wp-login.php)
-   [Entries
    RSS](https://tropicaldevel.wordpress.com/feed/ "Syndicate this site using RSS 2.0")
-   [Comments
    RSS](https://tropicaldevel.wordpress.com/comments/feed/ "The latest comments to all posts in RSS")

[Blog at WordPress.com](http://wordpress.com/?ref=footer). | Theme:
[Ocean
Mist](http://theme.wordpress.com/themes/ocean-mist/ "Learn more about this theme")
by [Ed Merritt](http://www.edmerritt.com/).

![](//pixel.quantserve.com/pixel/p-18-mFEk4J448M.gif?labels=%2Clanguage.en%2Ctype.wpcom%2Cas)
...
![](https://stats.wordpress.com/b.gif?v=noscript)
